...
import Add from "@/app/[locale]/accounting/ui/ledgers/add";
import Pagination from "@/app/[locale]/accounting/ui/ledgers/pagination";
import List from "@/app/[locale]/accounting/ui/ledgers/list";
import Export from "@/app/[locale]/accounting/ui/ledgers/export";
import Filter from "@/app/[locale]/accounting/ui/ledgers/filter";
import Search from "@/app/[locale]/accounting/ui/ledgers/search";
import Loader from "@/components/common/Loader";
import SetPage from "@/app/[locale]/accounting/ui/ledgers/page";
...
async function Index({
  ...
  if (dataFilter) {
    ...
    dataFilter = {order_by:'register',order_by_dir:'DESC',register_start:'',register_end:'',type:'',status:''};
  }
  ...
  const listData = await list(process.env.API_ACCOUNTING_LEDGERS as string, query); 
  ...
  const accounts = await list(process.env.API_ACCOUNTING_ACCOUNTS as string, queryAccount);
  ...
  return (
    <>
      ...
      <div>
        <Breadcrumb pageName={messagesIntl.accounting.ledgers.label} />
        ...
        <div className=" bg-white rounded-lg p-10 dark:bg-boxdark">
          <div className="flex justify-start gap-4.5 py-2">
            {access && access.includes('account_create') &&
              <Add messagesIntl={messagesIntl} accounts={accounts.data} />
            }
          ...
          <label className="text-lg font-bold my-4 block">Data {messagesIntl.accounting.ledgers.label}</label>
          <div className="overflow-x-auto mx-auto">
            <Suspense fallback={<CardSkeleton />}>
              <List messagesIntl={messagesIntl} rows={rows} accounts={accounts.data} listInc={listInc} />
            </Suspense>
