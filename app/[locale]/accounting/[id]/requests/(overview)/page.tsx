...
import Pagination from "@/app/[locale]/accounting/ui/requests/pagination";
import List from "@/app/[locale]/accounting/ui/requests/list";
import Export from "@/app/[locale]/accounting/ui/requests/export";
import Filter from "@/app/[locale]/accounting/ui/requests/filter";
import Search from "@/app/[locale]/accounting/ui/requests/search";
import Loader from "@/components/common/Loader";
import SetPage from "@/app/[locale]/accounting/ui/requests/page";
...
async function Index({
  ...
  if (dataFilter) {
    ...
    dataFilter = {order_by:'created_at',order_by_dir:'DESC',register_start:'',register_end:'',type:'',status:''};
  }
  ...
  if (dataFilter) {
    dataQry += '&register_start=' + dataFilter.register_start + '&register_end=' + dataFilter.register_end + '&type=' + dataFilter.type + '&status=' + dataFilter.status + '&order_by=' + dataFilter.order_by + '&order_by_dir=' + dataFilter.order_by_dir + '&module=accounting';
  }
  const query = '/?page=' + currentPage + "&per_page=" + perPage + dataQry

  const listData = await list(process.env.API_CAPITAL_REQUESTS as string, query); 
    ...
    <>
      ...
      <div>
        ...
        <div className=" bg-white rounded-lg p-10 dark:bg-boxdark">
          ...
          <label className="text-lg font-bold my-4 block">Data {messagesIntl.user.requests.title}</label>
