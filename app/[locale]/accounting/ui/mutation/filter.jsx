...
export default function Filter({ dataFilter, messagesIntl }) {
    ...
    const [form, setForm] = useState({
        ...
        status: dataFilter.status ? dataFilter.status : 'active',
        type: dataFilter.type ? dataFilter.type : '',
        order_by: dataFilter.order_by ? dataFilter.order_by : 'register',
        order_by_dir: dataFilter.order_by_dir ? dataFilter.order_by_dir : 'ASC',
        account_id: dataFilter.account_id ? dataFilter.account_id : '',
    });
    ...
    const getData = async () => {
        const dataQry = '/?postable=Y' + '&order_by=code' + '&order_by_dir=ASC'
        const data = await list(process.env.API_ACCOUNTING_ACCOUNTS, dataQry);
        console.log('API_ACCOUNTING_ACCOUNTS...', data)
                        ...
                        <div className="">
                            ...
                            <form onSubmit={handleSubmit}>
                                ...
                                <div className="form-control">
                                    <label className="pt-5 mb-3 block text-black dark:text-white">{messagesIntl.accounting.accounts.name}</label>
                                    <select onChange={(e) => handleForm('account_id', e.target.value)} className="relative z-20 w-full appearance-none rounded border border-stroke bg-transparent py-3 px-5 outline-none transition focus:border-primary active:border-primary dark:border-form-strokedark dark:bg-form-input">
                                        <option value='' selected={(form.account_id == '') ? 'selected' : ''}>Select Account</option>
                                        {accounts?.map((val,index) => {
                                            return <option key={index} value={val.id} selected={(form.account_id == val.id) ? 'selected' : ''}>{val.code} - {val.name}</option>
                                ...
                                <div className="flex flex-row gap-2">
                                    <select onChange={(e) => handleForm('order_by', e.target.value)} className="relative z-20 w-full appearance-none rounded border border-stroke bg-transparent py-3 px-5 outline-none transition focus:border-primary active:border-primary dark:border-form-strokedark dark:bg-form-input">
                                        ...
                                        <option value='register' selected={(form.order_by == 'register') ? 'selected' : ''}>{messagesIntl.accounting.ledgers.register}</option>
                                        <option value='memo' selected={(form.order_by == 'memo') ? 'selected' : ''}>{messagesIntl.accounting.ledgers.memo}</option>
                                        <option value='code' selected={(form.order_by == 'code') ? 'selected' : ''}>{messagesIntl.accounting.ledgers.code}</option>
                                    </select>
