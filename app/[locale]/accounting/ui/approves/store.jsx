...
export default function Store({ messagesIntl, rows, dataParams }) {
  ...
  async function handleChange() {
    //if approved
    if (approved) {
      //update ledger status active
      let ledgerRequest = {
        id: dataParams.transaction_id,
        status: 'active'
      }
      const dataPatch = await patch(process.env.API_ACCOUNTING_LEDGER_PATCH, ledgerRequest)
      console.log('dataPatch activatedLedger', dataPatch)
      if (dataPatch.success) {
        ...
        let dataRequest = {
          ...
          status_trs_old: 'pending',
          status_trs_new: 'activate',
        }
  ...
  return (
    <div>
      <div class="px-4 sm:px-0">
        <h3 class="text-base font-semibold leading-7 text-gray-900">Ledger Information</h3>
        <p class="mt-1 max-w-2xl text-sm leading-6 text-gray-500">Ledger details.</p>
      </div>
