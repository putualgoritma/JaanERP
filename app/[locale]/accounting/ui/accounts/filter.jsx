                        ...
                        <div className="">
                            ...
                            <form onSubmit={handleSubmit}>
                                ...
                                <div className="form-control">
                                    <label className="pt-5 mb-3 block text-black dark:text-white">{messagesIntl.accounting.accounts.status}</label>
                                    <select onChange={(e) => handleForm('status', e.target.value)} className="relative z-20 w-full appearance-none rounded border border-stroke bg-transparent py-3 px-5 outline-none transition focus:border-primary active:border-primary dark:border-form-strokedark dark:bg-form-input">
                                        <option value='' selected={(form.status == '') ? 'selected' : ''}>{messagesIntl.accounting.accounts.status}</option>
                                ...
                                <div className="form-control">
                                    <label className="pt-5 mb-3 block text-black dark:text-white">{messagesIntl.accounting.accounts.type}</label>
                                    <select onChange={(e) => handleForm('type', e.target.value)} className="relative z-20 w-full appearance-none rounded border border-stroke bg-transparent py-3 px-5 outline-none transition focus:border-primary active:border-primary dark:border-form-strokedark dark:bg-form-input">
                                        <option value='' selected={(form.type == '') ? 'selected' : ''}>{messagesIntl.accounting.accounts.type}</option>
                                        <option value='Assets' selected={(form.type == 'Assets') ? 'selected' : ''}>{messagesIntl.accounting.accounts.type_option.Assets}</option>
                                        <option value='Liabilities' selected={(form.type == 'Liabilities') ? 'selected' : ''}>{messagesIntl.accounting.accounts.type_option.Liabilities}</option>
                                        <option value='Equity' selected={(form.type == 'Equity') ? 'selected' : ''}>{messagesIntl.accounting.accounts.type_option.Equity}</option>
                                        <option value='Revenues' selected={(form.type == 'Revenues') ? 'selected' : ''}>{messagesIntl.accounting.accounts.type_option.Revenues}</option>
                                        <option value='Expenses' selected={(form.type == 'Expenses') ? 'selected' : ''}>{messagesIntl.accounting.accounts.type_option.Expenses}</option>
                                    </select>
                                </div>

                                <div className="form-control">
                                    <label className="pt-5 mb-3 block text-black dark:text-white">{messagesIntl.accounting.accounts.register}</label>
                                ...
                                <div className="flex flex-row gap-2">
                                    <select onChange={(e) => handleForm('order_by', e.target.value)} className="relative z-20 w-full appearance-none rounded border border-stroke bg-transparent py-3 px-5 outline-none transition focus:border-primary active:border-primary dark:border-form-strokedark dark:bg-form-input">
                                        ...
                                        <option value='name' selected={(form.order_by == 'name') ? 'selected' : ''}>{messagesIntl.accounting.accounts.name}</option>
                                        <option value='code' selected={(form.order_by == 'code') ? 'selected' : ''}>{messagesIntl.accounting.accounts.code}</option>
                                        <option value='status' selected={(form.order_by == 'status') ? 'selected' : ''}>{messagesIntl.accounting.accounts.status}</option>
                                        <option value='type' selected={(form.order_by == 'type') ? 'selected' : ''}>{messagesIntl.accounting.accounts.type}</option>
                                    </select>
