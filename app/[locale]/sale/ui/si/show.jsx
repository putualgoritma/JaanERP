...
export default function Show({ dataUpd }) {
    //console.log('show dataUpd...', dataUpd)
                ...
                <div className="modal-box w-7/12 max-w-5xl no-scrollbar overflow-hidden  dark:bg-boxdark">
                    ...
                    <div className="">
                        ...
                        <div className={`overflow-y-scroll h-75 custom-scrollbar ${tab === 1 ? "" : "hidden"}`}>
                            ...
                            <label className="pt-5 mb-3 block text-black dark:text-white font-semibold ">{intl.user.users.title} </label>

                            <div className="grid grid-cols-2">
                                <div className="form-control">
                                    <label className="pt-5 mb-3 block text-black dark:text-white font-semibold ">{intl.user.users.name} </label>
                                    <p className="ml-3 block text-black dark:text-white">{data.users[0]?.contact?.name}</p>
                                </div>
                                <div className="form-control">
                                    <label className="pt-5 mb-3 block text-black dark:text-white font-semibold ">{intl.user.users.phone} </label>
                                    <p className="ml-3 block text-black dark:text-white">{data.users[0]?.contact?.phone}</p>
                                </div>
                                <div className="form-control col-span-2">
                                    <label className="pt-5 mb-3 block text-black dark:text-white font-semibold ">{intl.user.users.address} </label>
                                    <p className="ml-3 block text-black dark:text-white">{data.users[0]?.contact?.address}</p>
                                </div>
                            </div>


                            {data.transaction_product.length > 0 &&
                                <div className="form-control">
                                    ...
                                    {data.transaction_product.map((product,index) => {
                                        return <div key={index} className="grid grid-cols-5 gap-0">
