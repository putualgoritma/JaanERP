...
import Add from "@/app/[locale]/hr/ui/staffs/add";
import Pagination from "@/app/[locale]/hr/ui/staffs/pagination";
import List from "@/app/[locale]/hr/ui/staffs/list";
import Export from "@/app/[locale]/hr/ui/staffs/export";
import Filter from "@/app/[locale]/hr/ui/staffs/filter";
import Search from "@/app/[locale]/hr/ui/staffs/search";
import Loader from "@/components/common/Loader";
import SetPage from "@/app/[locale]/hr/ui/staffs/setpage";
...
async function Index({
  ...
  if (dataFilter) {
    ...
    dataFilter = { register_start: '', register_end: '', order_by: 'register', order_by_dir: 'DESC', status: '', type: '' };
  }
  ...
  const query = '/?page=' + currentPage + "&per_page=" + perPage + dataQry + '&type=staff'

  const listData = await list(process.env.API_USER_STAFFS as string, query);
  ...
  const queryRoles = ''
  const listRoles = await list(process.env.API_USER_ROLES as string, queryRoles);  
  const roles = listRoles.data;
  ...
  return (
    <>
      ...
      <div>
        <Breadcrumb pageName={messagesIntl.user.staffs.title} />
        ...
        <div className=" bg-white rounded-lg p-10 dark:bg-boxdark">
          ...
          <label className="text-lg font-bold my-4 block">Data {messagesIntl.user.staffs.title}</label>
          <div className="overflow-x-auto mx-auto">
            <Suspense fallback={<CardSkeleton />}>
              <List messagesIntl={messagesIntl} rows={rows} roles={roles} />
            </Suspense>
