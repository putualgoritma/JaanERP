...
export default function Filter({ dataFilter, messagesIntl }) {
    ...
    const getData = async () => {
        ...
        const query = '/' + dataQry
        const data = await list(process.env.API_TRANSACTIONS_GET_SUPPLIER, query)
                        ...
                        <div className="">
                            ...
                            <form onSubmit={handleSubmit}>
                                ...
                                <div className="form-control">
                                    <label className="pt-5 mb-3 block text-black dark:text-white">{messagesIntl.trades.suppliers.title}</label>
                                    <select onChange={(e) => handleForm('user_id', e.target.value)} className="relative z-20 w-full appearance-none rounded border border-stroke bg-transparent py-3 px-5 outline-none transition focus:border-primary active:border-primary dark:border-form-strokedark dark:bg-form-input">
                                        ...
                                        {users?.map((val,index) => {
                                            return <option key={index} value={val.id} selected={(form.user_id == val.id) ? 'selected' : ''}>{val.contact.name}</option>
