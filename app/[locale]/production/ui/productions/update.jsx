...
import { addPrd, initBom, updBom, SetPath, initPrd, initDataPrd, updPrd, delPrd } from '@/app/[locale]/inventory/lib/services';
...
export default function Update({ dataUpd }) {
    ...
    const [data, setData] = useState(initDataPrd('productions', dataUpd.row));
    const searchParams = useSearchParams();
    const pathname = SetPath();

    function resetData() {
        setData(initDataPrd('productions'));
    }
    ...
    async function handleSubmit(e) {
        ...
        const dataUpdate = await store(process.env.API_TRANSACTIONS_PRODUCTION_UPDATE, data)
        setIsMutating(false)
        resetData()
        setModal(false)
                    ...
                    <div className="no-scrollbar overflow-hidden">
                        ...
                        <form onSubmit={handleSubmit} className="group">
                            ...
                            <div className={`overflow-y-scroll h-80 custom-scrollbar ${tab === 1 ? "" : "hidden"}`}>
                                ...
                                <div className="form-control">
                                    <label className="pt-5 mb-3 block text-black dark:text-white">{messagesIntl.trades.staffs.title} <span className="text-meta-1">*</span></label>
                                    <div className="flex gap-2">
                                        <input
                                            ref={cUserRef}
                                            value={data.supplier.name}
                                            type="text" className="w-full rounded-lg border-[1.5px] border-stroke bg-transparent py-3 px-5 font-medium outline-none transition focus:border-primary active:border-primary disabled:cursor-default disabled:bg-whiter dark:border-form-strokedark dark:bg-form-input dark:focus:border-primary" placeholder="select staff..." required readOnly />
                                        {modal &&
                                            <InsertUser messagesIntl={messagesIntl} setData={setData} data={data} type='staff' />
                                        }
                                    </div>
                                </div>
                                ...
                                {data.products.map((val, key) => {
                                    return (
                                        <div key={key} className="grid grid-cols-4 gap-1 py-1 ">

                                            {/* Material */}
                                            {modal &&
                                                <InsertProduct messagesIntl={messagesIntl} setData={setData} data={data} initData={initPrd} keyco={key} valco={val} type='manufacturing' />
                                            }
                                            <input
                                                value={val.qty}
                                                onChange={(e) => updPrd(setData, data, key, e.target.value, "qty", val)}
                                            ...
                                            <div className="w-full border-[1.5px] cursor-pointer border-stroke bg-transparent py-3 px-5 text-center font-medium outline-none bg-red-500 text-white" onClick={() => { delPrd(setData, data, key) }}>
                                ...
                                <div className="w-1/3 border-[1.5px] border-current flex items-center justify-center rounded mt-2 py-2 px-2 text-sky-500 hover:bg-opacity-95 gap-2 cursor-pointer" onClick={() => { addPrd(setData, data) }}>
                                ...
                                {data.products.map((valp, keyp) => {
                                    return (
                                        <div key={keyp}>
                                            <label className="pt-5 mb-3 block text-black dark:text-white">{messagesIntl.trades.product_materials.title} {valp.name} <span className="text-meta-1">*</span></label>
                                            ...
                                            {valp.product_materials.map((val, key) => {
                                                return (
                                                    <div key={key} className="grid grid-cols-3 gap-1 py-1 ">
                                                        ...
                                                        <input
                                                            value={val.qty}
                                                            onChange={(e) => updBom(setData, data, keyp, key, e.target.value, "qty")}
                                                            type="number" className="w-full border-[1.5px] border-stroke bg-transparent py-3 px-5 font-medium outline-none transition focus:border-primary active:border-primary disabled:cursor-default disabled:bg-whiter dark:border-form-strokedark dark:bg-form-input dark:focus:border-primary invalid:[&:not(:placeholder-shown):not(:focus)]:border-red-500 peer" placeholder="" required />
                                                    </div>)
                                            })}
                                        </div>
                                ...
                                {data.products.length > 1 &&
                                    <>
                                        <label className="pt-5 mb-3 block text-black dark:text-white">Total {messagesIntl.trades.product_materials.title}</label>
                                        <div className="grid grid-cols-3 gap-1 py-1">
                                        ...
                                        {data.product_inventories.map((val, key) => {
                                            return (
                                                <div key={key} className="grid grid-cols-3 gap-1 py-1 ">
                                                    ...
                                                    <input
                                                        value={val.qty}
                                                        type="number" className="w-full border-[1.5px] border-stroke bg-transparent py-3 px-5 font-medium outline-none transition focus:border-primary active:border-primary disabled:cursor-default disabled:bg-whiter dark:border-form-strokedark dark:bg-form-input dark:focus:border-primary invalid:[&:not(:placeholder-shown):not(:focus)]:border-red-500 peer" placeholder="" required disabled />
                                                </div>)
                                        })
                                        }</>}
