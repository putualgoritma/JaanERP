...
export default function Update({ dataUpd }) {
    ...
    async function handleSubmit(e) {
        ...
        const dataUser = await update(process.env.API_USER_PERMISSION_UPDATE, dataRequest)
        setIsMutating(false)
        resetData()
        // router.refresh();
        setModal(false)
                        ...
                        <div className="">
                            <div className="border-b border-stroke py-4 dark:border-strokedark">
                                <h3 className="font-medium text-black dark:text-white">
                                    {messagesIntl.input.button.update} {messagesIntl.user.permissions.title}
                                </h3>
                            </div>

                            <div className="grid grid-flow-col text-center text-gray-500 bg-gray-100 p-1 dark:bg-meta-4">
                                <button onClick={() => { setTab(1) }} className={`${tab === 1 ? 'bg-white rounded-full shadow text-indigo-900' : ''} flex justify-center py-2 `}>Permission</button>
                            </div>
                            ...
                            <form onSubmit={handleSubmit} className="group">
                                ...
                                <div className={`overflow-y-scroll h-75 custom-scrollbar ${tab === 1 ? "" : "hidden"}`}>
                                    ...
                                    <div className="form-control">
                                        <label className="pt-5 mb-3 block text-black dark:text-white">{messagesIntl.user.permissions.title} <span className="text-meta-1">*</span></label>
                                        <input
                                            value={data.title}
