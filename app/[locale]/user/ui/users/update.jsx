...
export default function Update({dataUpd}) {
    ...
    async function handleSubmit(e) {
        ...
        const dataUpdate = await update(process.env.API_USER_UPDATE, dataRequest)
        setIsMutating(false)
        resetData()
        // router.refresh();
        setModal(false)
                ...
                <div className="modal-box w-7/12 max-w-5xl no-scrollbar overflow-hidden  dark:bg-boxdark">
                    ...
                    <div className="">
                        <div className="border-b border-stroke py-4 dark:border-strokedark">
                            ...
                            {messagesIntl.input.button.update} {messagesIntl.user.users.title}
                            </h3>
                        </div>
                        ...
                        <form onSubmit={handleSubmit} className="group">
                            ...
                            <div className={`overflow-y-scroll h-75 custom-scrollbar ${tab === 1 ? "" : "hidden"}`}>
                                <div className="form-control">
                                    <label className="pt-5 mb-3 block text-black dark:text-white">{messagesIntl.user.users.email}</label>
                                    <input
                                        value={data.email}
                                ...
                                <div className="form-control">
                                    ...
                                    <select onChange={(e) => setData({ ...data, type: e.target.value })} className="relative z-20 w-full appearance-none rounded border border-stroke bg-transparent py-3 px-5 outline-none transition focus:border-primary active:border-primary dark:border-form-strokedark dark:bg-form-input" required>
                                        ...
                                        {data.type == 'member' ?
                                            <option value={"member"} disabled selected>{messagesIntl.user.users.type_option.member}</option>
                                            :
                                            <option value={"member"}>{messagesIntl.user.users.type_option.member}</option>
                                        }
                                        {data.type == 'staff' ?
                                            <option value={"staff"} disabled selected>{messagesIntl.user.users.type_option.staff}</option>
                                            :
                                            <option value={"staff"}>{messagesIntl.user.users.type_option.staff}</option>
                                        }
                                        {data.type == 'admin' ?
                                            <option value={"admin"} disabled selected>{messagesIntl.user.users.type_option.admin}</option>
                                            :
                                            <option value={"admin"}>{messagesIntl.user.users.type_option.admin}</option>
                                        }
                                        {data.type == 'supplier' ?
                                            <option value={"supplier"} disabled selected>{messagesIntl.user.users.type_option.supplier}</option>
                                            :
                                            <option value={"supplier"}>{messagesIntl.user.users.type_option.supplier}</option>
                                        }
                                        {data.type == 'public' ?
                                            <option value={"public"} disabled selected>{messagesIntl.user.users.type_option.public}</option>
                                            :
                                            <option value={"public"}>{messagesIntl.user.users.type_option.public}</option>
                                        }
                                    </select>
                                </div>

                                <div className="form-control">
                                        <label className="pt-5 mb-3 block text-black dark:text-white">{messagesIntl.user.users.role_id} <span className="text-meta-1">*</span></label>
                                        <select onChange={(e) => setData({ ...data, role_id: e.target.value })} className="relative z-20 w-full appearance-none rounded border border-stroke bg-transparent py-3 px-5 outline-none transition focus:border-primary active:border-primary dark:border-form-strokedark dark:bg-form-input" required>
                                            <option value="" disabled selected={(data.role_id == '') ? 'selected' : ''}>{messagesIntl.user.users.role_id}</option>
