...
export default function InsertUser({ setData, data, messagesIntl, type }) {
    ...
    async function fetchdashboard() {
        ...
        dataQry += '?keyword=' + search.keyword + "&per_page=10" + "&page=1" + qryType;
        const query = '/' + dataQry
        const data = await list(process.env.API_USERS, query)
        //console.log('list unit', data)
    ...
    return (
        <>
            <div className="cursor-pointer flex items-center justify-center rounded bg-primary py-3 px-6 text-white hover:bg-opacity-95 gap-2" onClick={initClick} >
                    ...
                    <div className="modal-box w-11/12 max-w-5xl dark:bg-boxdark no-scrollbar overflow-hidden">
                        ...
                        <div className="border-b border-stroke py-2 dark:border-strokedark">
                            <h3 className="font-medium text-black dark:text-white">
                                {messagesIntl ? messagesIntl.input.button.list : ''} {messagesIntl ? messagesIntl.trades.suppliers.title : ''}
                            </h3>
                        </div>
                            ...
                            <table className="w-full table-auto">
                                <thead>
                                    <tr className="bg-gray-2 text-left dark:bg-meta-4">
                                        ...
                                        <th className="py-4 px-2 font-medium text-black dark:text-white">
                                            {messagesIntl ? messagesIntl.trades.suppliers.name : ''}
                                        </th>
                                    </tr>
                                </thead>
                                <tbody>
                                    {inserts.map((insert, index) => (
                                        <tr onClick={() => {
                                            setData({
                                                ...
                                                supplier: {
                                                    ...data.supplier,
                                                    name: insert.contact ? insert.contact.name : '',
                                                },
                                                user: {
                                                    ...data.user,
                                                    name: insert.contact ? insert.contact.name : '',
                                            ...
                                            <td className="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
                                                <p className="text-black dark:text-white">
                                                    {insert.contact ? insert.contact.name : ''}
                                                </p>
                                            </td>
                                        </tr>
                                    ))}
