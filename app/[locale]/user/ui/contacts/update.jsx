...
export default function Update({ dataUpd }) {
  ...
  const [data, setData] = useState({
    ...
    card_id: dataUpd.row.card_id,
    birthday: dataUpd.row.birthday,
    sex: dataUpd.row.sex,
    email: dataUpd.row.email,
    phone: dataUpd.row.phone,
    description: dataUpd.row.description
  });
  ...
  async function handleSubmit(e) {
    ...
    const dataUpdate = await update(process.env.API_CONTACT_UPDATE, dataRequest);
              ...
              <form onSubmit={handleSubmit} className="group">
                ...
                <div className={`overflow-y-scroll h-75 custom-scrollbar ${tab === 1 ? "" : "hidden"}`}>
                  ...
                  <div className="form-control">
                    <label className="pt-5 mb-3 block text-black dark:text-white">{intl.user.contacts.name} <span className="text-meta-1">*</span></label>
                    <input
                      value={data.name}
                  ...
                  <div className="form-control">
                    <label className="pt-5 mb-3 block text-black dark:text-white">{intl.user.contacts.card_id} <span className="text-meta-1">*</span></label>
                    <input
                      value={data.card_id}
                  ...
                  <div className="form-control">
                    <label className="pt-5 mb-3 block text-black dark:text-white">{intl.user.users.email} <span className="text-meta-1">*</span></label>
                    <input
                      value={data.email}
                  ...
                  <div className="form-control">
                    <label className="pt-5 mb-3 block text-black dark:text-white">{intl.user.contacts.birthday}</label>
                    <input
                      value={data.birthday}
                  ...
                  <div className="form-control">
                    <label className="pt-5 mb-3 block text-black dark:text-white">{intl.user.contacts.phone} <span className="text-meta-1">*</span></label>
                    <input
                      value={data.phone}
                  ...
                  <div className="form-control">
                    <label className="pt-5 mb-3 block text-black dark:text-white">{intl.user.contacts.sex} <span className="text-meta-1">*</span></label>
                    <select onChange={(e) => setData({ ...data, sex: e.target.value })} className="relative z-20 w-full appearance-none rounded border border-stroke bg-transparent py-3 px-5 outline-none transition focus:border-primary active:border-primary dark:border-form-strokedark dark:bg-form-input" required>
                      <option value='' selected={(data.sex == '') ? 'selected' : ''}>{intl.user.contacts.sex}</option>
                      <option value='M' selected={(data.sex == 'M') ? 'selected' : ''}>{intl.user.contacts.sex_option.M}</option>
                      <option value='F' selected={(data.sex == 'F') ? 'selected' : ''}>{intl.user.contacts.sex_option.F}</option>
                    </select>
                  </div>
