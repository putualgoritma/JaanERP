...
import SetPage from "@/app/[locale]/user/ui/contacts/setpage";
...
async function Index({
  ...
  if (dataFilter) {
    dataQry += '&register_start=' + dataFilter.register_start + '&register_end=' + dataFilter.register_end + '&order_by=' + dataFilter.order_by + '&order_by_dir=' + dataFilter.order_by_dir;
  }
  const query ='?page=' + currentPage + "&per_page=" + perPage + dataQry
  console.log('query', query)

  const listData = await list(process.env.API_CONTACTS as string, query);
  ...
  return (
    <>
      ...
      <div>
        <Breadcrumb pageName={messagesIntl.user.contacts.title} />
        ...
        <div className=" bg-white rounded-lg p-10 dark:bg-boxdark">
          ...
          <label className="text-lg font-bold my-4 block">Data {messagesIntl.user.contacts.title}</label>
