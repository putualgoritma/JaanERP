...
import Add from "@/app/[locale]/user/ui/roles/add";
import Pagination from "@/app/[locale]/user/ui/roles/pagination";
import List from "@/app/[locale]/user/ui/roles/list";
import Export from "@/app/[locale]/user/ui/roles/export";
import Filter from "@/app/[locale]/user/ui/roles/filter";
import Search from "@/app/[locale]/user/ui/roles/search";
import Loader from "@/components/common/Loader";
import SetPage from "@/app/[locale]/user/ui/roles/setpage";
...
async function Index({
  ...
  if (dataFilter) {
    ...
    dataFilter = {order_by:'title',order_by_dir:'ASC',register_start:'',register_end:'',type:'',status:''};
  }
  ...
  const listData = await list(process.env.API_USER_ROLES as string, query); 
  ...
  const queryPermissions = '/?order_by=title&order_by_dir=ASC'
  const permissions = await list(process.env.API_USER_PERMISSIONS as string, queryPermissions);
  ...
  return (
    <>
      ...
      <div>
        <Breadcrumb pageName={messagesIntl.user.roles.title} />

        <div className=" bg-white rounded-lg p-10 dark:bg-boxdark">
          <div className="flex justify-start gap-4.5 py-2">
            {access && access.includes('role_create') &&
              <Add messagesIntl={messagesIntl} rows={permissions.data} />
            }
          ...
          <label className="text-lg font-bold my-4 block">Data {messagesIntl.user.roles.title}</label>
          <div className="overflow-x-auto mx-auto">
            <Suspense fallback={<CardSkeleton />}>
              <List messagesIntl={messagesIntl} rows={rows} permissions={permissions.data} />
            </Suspense>
