...
import Add from "@/app/[locale]/capital/ui/users/add";
import Pagination from "@/app/[locale]/capital/ui/users/pagination";
import List from "@/app/[locale]/capital/ui/users/list";
import Export from "@/app/[locale]/capital/ui/users/export";
import Filter from "@/app/[locale]/capital/ui/users/filter";
import Search from "@/app/[locale]/capital/ui/users/search";
import Loader from "@/components/common/Loader";
import SetPage from "@/app/[locale]/capital/ui/users/page";
...
async function Index({
  ...
  if (dataFilter) {
    ...
    dataFilter = { register_start: '', register_end: '', order_by: 'register', order_by_dir: 'DESC', status: '' };
  }
  ...
  if (dataFilter) {
    dataQry += '&register_start=' + dataFilter.register_start + '&register_end=' + dataFilter.register_end + '&status=' + dataFilter.status + '&order_by=' + dataFilter.order_by + '&order_by_dir=' + dataFilter.order_by_dir;
  }
  const query = '/?page=' + currentPage + "&per_page=" + perPage + dataQry + '&type=member'
  //console.log("query...: ", query)

  const listData = await list(process.env.API_CAPITAL_USERS as string, query);
  ...
  //list all account
  const queryAccount = '/?postable=Y' + '&order_by=code' + '&order_by_dir=ASC'
  //console.log("query Account...: ", queryAccount)
  let accounts = await list(process.env.API_ACCOUNTING_ACCOUNTS as string, queryAccount);
  ...
  return (
    <>
      ...
      <div>
        <Breadcrumb pageName={messagesIntl.user.users.type_option.member} />
        ...
        <div className=" bg-white rounded-lg p-10 dark:bg-boxdark">
          ...
          <label className="text-lg font-bold my-4 block">Data {messagesIntl.user.users.type_option.member}</label>
          <div className="overflow-x-auto mx-auto">
            <Suspense fallback={<CardSkeleton />}>
              <List messagesIntl={messagesIntl} rows={rows} accounts={accounts.data} />
            </Suspense>
