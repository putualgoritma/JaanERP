...
export default function Update({ dataUpd }) {
    ...
    async function handleSubmit(e) {
        ...
        const dataUpdate = await update(process.env.API_CAPITAL_USER_UPDATE_PROFILE, dataRequest)
        setIsMutating(false)
        resetData()
        // router.refresh();
        setModal(false)
                        ...
                        <div className="">
                            <div className="border-b border-stroke py-4 dark:border-strokedark">
                                <h3 className="font-medium text-black dark:text-white">
                                    {messagesIntl.input.button.update} {messagesIntl.user.users.type_option.member}
                                </h3>
                            </div>
                            ...
                            <form onSubmit={handleSubmit} className="group">
                                ...
                                <div className={`overflow-y-scroll h-75 custom-scrollbar ${tab === 2 ? "" : "hidden"}`}>
                                    ...
                                    <div className="form-control">
                                        ...
                                        <select onChange={(e) => setContact({ ...contact, sex: e.target.value })} className="relative z-20 w-full appearance-none rounded border border-stroke bg-transparent py-3 px-5 outline-none transition focus:border-primary active:border-primary dark:border-form-strokedark dark:bg-form-input" required>
                                            <option value='' selected={(contact.sex == '') ? 'selected' : ''}>{messagesIntl.user.contacts.sex}</option>
                                            <option value='M' selected={(contact.sex == 'M') ? 'selected' : ''}>{messagesIntl.user.contacts.sex_option.M}</option>
                                            <option value='F' selected={(contact.sex == 'F') ? 'selected' : ''}>{messagesIntl.user.contacts.sex_option.F}</option>
                                        </select>
                                    </div>
