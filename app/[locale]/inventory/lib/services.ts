...
export const updRelTrans = (setData: any, data: any, val: any) => {
    ...
    val.transaction_product.map((obj: any) => {
        ...
        let dat = { name: obj.code + ' - ' + obj.name, qty: obj.pivot.qty, unit: obj.units.name, id: obj.id, price: obj.pivot.price, cogs: obj.pivot.cogs, priceLabel: formatNumber(obj.pivot.price), subTotal: formatNumber(subTotal), type: obj.type, out: obj.pivot.qty, in: 0, reject: 0, description: '', show: showval }
...
function initProducts(data: any) {
    ...
    data.map((obj: any) => {
        ...
        let dat = { name: obj.code + ' - ' + obj.name, qty: obj.pivot.qty, unit: obj.units.name, id: obj.id, price: obj.price, cogs: obj.cogs, priceLabel: formatNumber(obj.price), subTotal: formatNumber(subTotal), type: obj.type, product_materials: materials, out: obj.pivot.qty, in: 0, reject: 0, description: '', show: false }
        product_cart.push(dat);
    });
    return product_cart
}
...
export const addCart = (setData: any, data: any) => {
    let dat = { name: '', qty: '1', unit: '', id: '', price: '', cogs: '', priceLabel: '', subTotal: '', type: '', out: 1, in: 0, reject: 0, description: '', show: false }
    setData({ ...data, products: [...data.products, dat] })
}
