...
import Add from "@/app/[locale]/inventory/ui/products/add";
import Pagination from "@/app/[locale]/inventory/ui/products/pagination";
import List from "@/app/[locale]/inventory/ui/products/list";
import Export from "@/app/[locale]/inventory/ui/products/export";
import Filter from "@/app/[locale]/inventory/ui/products/filter";
import Search from "@/app/[locale]/inventory/ui/products/search";
import Loader from "@/components/common/Loader";
import SetPage from "@/app/[locale]/inventory/ui/products/page";
...
async function Index({
  ...
  if (dataFilter) {
    dataQry += '&price_start=' + dataFilter.price_start + '&price_end=' + dataFilter.price_end + '&type=' + dataFilter.type + '&status=' + dataFilter.status + '&order_by=' + dataFilter.order_by + '&order_by_dir=' + dataFilter.order_by_dir+'&warehouse_id=' + dataFilter.warehouse_id;
  }
  const query = '/?page=' + currentPage + "&per_page=" + perPage + dataQry

  const listData = await list(process.env.API_PRODUCTS as string, query);
  console.log('listData...', listData)
  const totalData = listData.data.total;
  const members = listData.data.data;
  let totalPages = 1;
  if(perPage > 0){
  ...
  //get product code
  const productCode = await list(process.env.API_PRODUCTS_GET_CODE as string, '');
        ...
        <div className=" bg-white rounded-lg p-10 dark:bg-boxdark">
          <div className="flex justify-start gap-4.5 py-2">
            {/* {access && access.includes('user_create') && */}
              <Add messagesIntl={messagesIntl} initData={{'productCode':productCode.data}} />
          ...
          <label className="text-lg font-bold my-4 block">Data {messagesIntl.trades.products.title}</label>
          <div className="overflow-x-auto mx-auto">
            <Suspense fallback={<CardSkeleton />}>
              <List messagesIntl={messagesIntl} products={members} />
            </Suspense>
