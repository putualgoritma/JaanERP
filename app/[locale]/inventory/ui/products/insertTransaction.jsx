...
export default function InsertTransaction({ setData, data, messagesIntl, updRelTrans, type }) {
    ...
    async function fetchdashboard() {
        setLoading(true)
        let dataQry = '';
        dataQry += '?keyword=' + search.keyword + "&per_page=10" + "&page=1" + "&type=" + type;
        const query = '/' + dataQry
        const data = await list(process.env.API_TRANSACTIONS, query)
        console.log('list API_TRANSACTIONS', data)
    ...
    return (
        <>
            <div className="cursor-pointer flex items-center justify-center rounded bg-secondary py-3 px-6 text-white hover:bg-opacity-95 gap-2" onClick={initClick} >
                    ...
                    <div className="modal-box w-11/12 max-w-5xl dark:bg-boxdark no-scrollbar overflow-hidden">
                        ...
                        <div className="border-b border-stroke py-2 dark:border-strokedark">
                            <h3 className="font-medium text-black dark:text-white">
                                {messagesIntl ? messagesIntl.input.button.list : ''} {messagesIntl ? messagesIntl.trades.transactions.title : ''}
                            </h3>
                        </div>
                            ...
                            <table className="w-full table-auto">
                                <thead>
                                    <tr className="bg-gray-2 text-left dark:bg-meta-4">
                                        ...
                                        <th className="py-4 px-2 font-medium text-black dark:text-white">
                                            {messagesIntl ? messagesIntl.trades.transactions.code : ''}
                                        </th>
                                        <th className="py-4 px-2 font-medium text-black dark:text-white">
                                ...
                                <tbody>
                                    {inserts.map((insert, index) => (
                                        <tr onClick={() => {
                                            updRelTrans(setData, data, insert);
                                            setModal(false);
                                            ...
                                            <td className="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
                                                <p className="text-black dark:text-white">
                                                    <ul class="list-disc">
                                                        ...
                                                        {insert.transaction_product?.map((val,key) => {
