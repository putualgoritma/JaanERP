...
export default function Filter({ dataFilter, messagesIntl }) {
    ...
    const [form, setForm] = useState({
        price_start: dataFilter.price_start?dataFilter.price_start:'',
        price_end: dataFilter.price_end?dataFilter.price_end:'',
      
        order_by: dataFilter.order_by?dataFilter.order_by:'id',
        order_by_dir: dataFilter.order_by_dir?dataFilter.order_by_dir:'ASC',
        warehouse_id: dataFilter.warehouse_id?dataFilter.warehouse_id:'',
    });

    const getData = async () => {
        const query = '';
        const data = await list(process.env.API_PRODUCTS_GET_WAREHOUSE, query)
        console.log('API_PRODUCTS_GET_WAREHOUSE', data)
                ...
                <div className="bg-white p-5 dark:border-strokedark dark:bg-boxdark">
                    <div className="">
                        ...
                        <div className="">
                            ...
                            <label className="pt-5 mb-3 block text-black dark:text-white">{messagesIntl.trades.warehouses.title}</label>
                                <select onChange={(e) => handleForm('warehouse_id', e.target.value)} className="relative z-20 w-full appearance-none rounded border border-stroke bg-transparent py-3 px-5 outline-none transition focus:border-primary active:border-primary dark:border-form-strokedark dark:bg-form-input">
                                        <option value='' selected={(form.warehouse_id == '') ? 'selected' : ''}>Select Column</option>
                                        {warehouses?.map((val,index)=>{
                                      return  <option key={index} value={val.id} selected={(form.warehouse_id == val.id) ? 'selected' : ''}>{val.name}</option>
                                ...
                                <div className="form-control">
                                    <label className="pt-5 mb-3 block text-black dark:text-white">{messagesIntl.Index.range} {messagesIntl.trades.products.price}</label>
                                    <div className="flex flex-row gap-2">
                                        <input
                                            value={form.price_start}
                                            onChange={(e) => handleForm('price_start', e.target.value)}
                                            type="number" className="w-full rounded-lg border-[1.5px] border-stroke bg-transparent py-3 px-5 font-medium outline-none transition focus:border-primary active:border-primary disabled:cursor-default disabled:bg-whiter dark:border-form-strokedark dark:bg-form-input dark:focus:border-primary" />

                                        <input
                                            value={form.price_end}
                                            onChange={(e) => handleForm('price_end', e.target.value)}
                                            type="number" className="w-full rounded-lg border-[1.5px] border-stroke bg-transparent py-3 px-5 font-medium outline-none transition focus:border-primary active:border-primary disabled:cursor-default disabled:bg-whiter dark:border-form-strokedark dark:bg-form-input dark:focus:border-primary" />
                                    </div>
                                </div>
                                ...
                                <div className="flex flex-row gap-2">
                                    <select onChange={(e) => handleForm('order_by', e.target.value)} className="relative z-20 w-full appearance-none rounded border border-stroke bg-transparent py-3 px-5 outline-none transition focus:border-primary active:border-primary dark:border-form-strokedark dark:bg-form-input">
                                        ...
                                        <option value='name' selected={(form.order_by == 'name') ? 'selected' : ''}>{messagesIntl.trades.products.name}</option>
                                        <option value='price' selected={(form.order_by == 'price') ? 'selected' : ''}>{messagesIntl.trades.products.price}</option>
                                    </select>
