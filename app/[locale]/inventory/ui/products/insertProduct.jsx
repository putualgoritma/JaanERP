...
export default function InsertProduct({ setData, data, initData, keyco, messagesIntl, valco, type }) {
    ...
    async function fetchdashboard() {
        ...
        const qry = "?keyword=" + search.keyword + "&page=1" + "&per_page=10" + "&type=" + type;
        const data = await list(process.env.API_PRODUCTS, qry)
        console.log('listData', data)
        setInserts(data.data.data)
        setLoading(false)
    }
    function initClick() {
        fetchdashboard()
    ...
    return (
        <>
            {!modal &&
                <select onClick={initClick} className="cursor-pointer col-span-2 border-[1.5px] border-stroke bg-transparent py-3 px-5 outline-none transition focus:border-primary active:border-primary dark:border-form-strokedark dark:bg-form-input" required>
                    <option value={valco.name}>{valco.name != '' ? valco.name : '...'}</option>
                </select>
            }
            <input type="checkbox" name="" checked={modal} onChange={handleChange} className="modal-toggle" id="" />
            <div className="modal z-9999999999999">
                {isLoading ?
                    <p>Loading...</p>
                    :
                    <div className="modal-box w-11/12 max-w-5xl dark:bg-boxdark no-scrollbar overflow-hidden">
                        ...
                        <div className="border-b border-stroke py-2 dark:border-strokedark">
                            <h3 className="font-medium text-black dark:text-white">
                                {messagesIntl ? messagesIntl.input.button.list : ''} {messagesIntl ? messagesIntl.trades.units.title : ''}
                            </h3>
                        </div>
                            ...
                            <table className="w-full table-auto">
                                <thead>
                                    <tr className="bg-gray-2 text-left dark:bg-meta-4">
                                        ...
                                        <th className="py-4 px-2 font-medium text-black dark:text-white">
                                            {messagesIntl ? messagesIntl.trades.products.name : ''}
                                        </th>
                                    </tr>
                                </thead>
                                <tbody>
                                    {inserts.map((insert, index) => (
                                        <tr onClick={() => {
                                            initData(setData, data, keyco, insert);
                                            setModal(false);
