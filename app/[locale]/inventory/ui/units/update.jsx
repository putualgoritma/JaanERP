...
export default function Update({ dataUpd }) {
    ...
    async function handleSubmit(e) {
        ...
        const dataUpdate = await store(process.env.API_UNITS_UPDATE, dataRequest)
        setIsMutating(false)
        resetData()
        // router.refresh();
        setModal(false)
                        ...
                        <form onSubmit={handleSubmit} className="group">
                            ...
                            <div className={`overflow-y-scroll h-75 custom-scrollbar ${tab === 1 ? "" : "hidden"}`}>
                                ...
                                <div className="form-control">
                                    <label className="pt-5 mb-3 block text-black dark:text-white">{intl?.trades.units.name} <span className="text-meta-1">*</span></label>
                                    <input
                                        value={data.name}
