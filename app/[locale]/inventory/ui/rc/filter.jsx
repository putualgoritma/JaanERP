...
export default function Filter({ dataFilter, messagesIntl }) {
    ...
    const [form, setForm] = useState({
        date_start: dataFilter.date_start ? dataFilter.date_start : '',
        date_end: dataFilter.date_end ? dataFilter.date_end : '',
        status: dataFilter.status ? dataFilter.status : '',
        order_by: dataFilter.order_by ? dataFilter.order_by : '',
        order_by_dir: dataFilter.order_by_dir ? dataFilter.order_by_dir : '',
        user_id: dataFilter.user_id ? dataFilter.user_id : '',
        product_id: dataFilter.product_id ? dataFilter.product_id : '',
    });
    ...
    const getData = async () => {
        let dataQry = '';
        dataQry += "?per_page=10" + "&page=1" + "&type=member";
        const query = '/' + dataQry
        console.log('query usrs...', query)
        const data = await list(process.env.API_USERS, query)
    ...
    const getProduct = async () => {
        let dataQry = '';
        dataQry += "?page=1" + "&per_page=10" + "&type=manufacturing";
        const query = '/' + dataQry
        console.log('query products...', query)
        const data = await list(process.env.API_PRODUCTS, query)
                        ...
                        <div className="">
                            ...
                            <form onSubmit={handleSubmit}>
                                ...
                                <div className="form-control">
                                    <label className="pt-5 mb-3 block text-black dark:text-white">{messagesIntl.user.users.title}</label>
                                    <select onChange={(e) => handleForm('user_id', e.target.value)} className="relative z-20 w-full appearance-none rounded border border-stroke bg-transparent py-3 px-5 outline-none transition focus:border-primary active:border-primary dark:border-form-strokedark dark:bg-form-input">
                                        <option value='' selected={(form.user_id == '') ? 'selected' : ''}>Select Column</option>
                                        {users?.map((val,index) => {
                                            return <option key={index} value={val.id} selected={(form.user_id == val.id) ? 'selected' : ''}>{val.contact?val.contact.name:'-'}</option>
                                ...
                                <div className="form-control">
                                    <label className="pt-5 mb-3 block text-black dark:text-white">{messagesIntl.trades.products.title}</label>
                                    <select onChange={(e) => handleForm('product_id', e.target.value)} className="relative z-20 w-full appearance-none rounded border border-stroke bg-transparent py-3 px-5 outline-none transition focus:border-primary active:border-primary dark:border-form-strokedark dark:bg-form-input">
                                        <option value='' selected={(form.product_id == '') ? 'selected' : ''}>Select Column</option>
                                        {products?.map((val,index) => {
                                            return <option key={index} value={val.id} selected={(form.product_id == val.id) ? 'selected' : ''}>{val.code} - {val.name}</option>
                                ...
                                <div className="form-control">
                                    <label className="pt-5 mb-3 block text-black dark:text-white">{messagesIntl.Index.range} {messagesIntl.trades.transactions.register}</label>
                                    <div className="flex flex-row gap-2">
                                        <input
                                            value={form.date_start}
                                            onChange={(e) => handleForm('date_start', e.target.value)}
                                        ...
                                        <input
                                            value={form.date_end}
                                            onChange={(e) => handleForm('date_end', e.target.value)}
                                            type="date" className="w-full rounded-lg border-[1.5px] border-stroke bg-transparent py-3 px-5 font-medium outline-none transition focus:border-primary active:border-primary disabled:cursor-default disabled:bg-whiter dark:border-form-strokedark dark:bg-form-input dark:focus:border-primary" />
                                    </div>
                                </div>

                                <div className="form-control">
                                    <label className="pt-5 mb-3 block text-black dark:text-white">Status</label>
                                    <select onChange={(e) => handleForm('status', e.target.value)} className="relative z-20 w-full appearance-none rounded border border-stroke bg-transparent py-3 px-5 outline-none transition focus:border-primary active:border-primary dark:border-form-strokedark dark:bg-form-input">
                                        <option value='' selected={(form.status == '') ? 'selected' : ''}>Status</option>
                                        <option value='pending' selected={(form.status == 'pending') ? 'selected' : ''}>Pending</option>
                                        <option value='success' selected={(form.status == 'success') ? 'selected' : ''}>Success</option>
                                ...
                                <div className="flex flex-row gap-2">
                                    <select onChange={(e) => handleForm('order_by', e.target.value)} className="relative z-20 w-full appearance-none rounded border border-stroke bg-transparent py-3 px-5 outline-none transition focus:border-primary active:border-primary dark:border-form-strokedark dark:bg-form-input">
                                        ...
                                        {/* <option value='name' selected={(form.order_by == 'name') ? 'selected' : ''}>{messagesIntl.trades.transactions.name}</option> */}
                                        <option value='register' selected={(form.order_by == 'register') ? 'selected' : ''}>{messagesIntl.trades.transactions.register}</option>
                                    </select>
