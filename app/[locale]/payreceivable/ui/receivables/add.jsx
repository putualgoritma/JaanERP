...
import { initData, SetPath } from '@/app/[locale]/payreceivable/lib/services';
import InsertUser from "@/app/[locale]/user/ui/users/insertUser";
import { InputNumberFormat } from '@react-input/number-format';
import { reFormatNumber } from '@/app/[locale]/lib/utils';

export default function Add({ messagesIntl, accounts, accountpays }) {
    const cUserRef = useRef(null);
    const [data, setData] = useState(initData('receivable'));
    ...
    async function handleSubmit(e) {
        ...
        const dataStore = await store(process.env.API_PAYRECEIVABLES_STORE, data)
        setIsMutating(false)
        resetData()
        setModal(false)
                        ...
                        <div className="no-scrollbar overflow-hidden">
                            <div className="border-b border-stroke py-2 dark:border-strokedark">
                                <h3 className="font-medium text-black dark:text-white">
                                    {messagesIntl.input.button.create} {messagesIntl.payreceivable.receivables.label}
                                </h3>
                            </div>

                            <div className="grid grid-flow-col text-center text-gray-500 bg-gray-100 p-1 dark:bg-meta-4">
                                <button onClick={() => { setTab(1) }} className={`${tab === 1 ? 'bg-white rounded-full shadow text-indigo-900' : ''} flex justify-center py-2 `}>{messagesIntl.payreceivable.receivables.label}</button>
                            </div>
                            ...
                            <form onSubmit={handleSubmit} className="group">
                                ...
                                <div className={`overflow-y-scroll h-75 custom-scrollbar ${tab === 1 ? "" : "hidden"}`}>
                                    ...
                                    <div className="form-control">
                                        <label className="pt-5 mb-3 block text-black dark:text-white">{messagesIntl.payreceivable.receivables.register} <span className="text-meta-1">*</span></label>
                                        <input
                                            value={data.register}
                                    ...
                                    <div className="form-control">
                                        <label className="pt-5 mb-3 block text-black dark:text-white">{messagesIntl.payreceivable.receivables.title} <span className="text-meta-1">*</span></label>
                                        <input
                                            value={data.title}
                                    ...
                                    <div className="form-control">
                                        <label className="pt-5 mb-3 block text-black dark:text-white">{messagesIntl.payreceivable.receivables.memo} <span className="text-meta-1">*</span></label>
                                        <textarea
                                            rows={6}
                                            placeholder=""
                                            value={data.memo}
                                    ...
                                    <div className="form-control">
                                        <label className="pt-5 mb-3 block text-black dark:text-white">{messagesIntl.payreceivable.receivables.user_id} <span className="text-meta-1">*</span></label>
                                        <div className="flex gap-2">
                                            ...
                                            {modal &&
                                                <InsertUser messagesIntl={messagesIntl} setData={setData} data={data} type='' />
                                            }
                                        </div>
                                    </div>

                                    <div className="form-control">
                                        <label className="pt-5 mb-3 block text-black dark:text-white">{messagesIntl.payreceivable.receivables.amount} <span className="text-meta-1">*</span></label>
                                        <InputNumberFormat
                                            locales="id-ID"
                                    ...
                                    <div className="form-control">
                                        <label className="pt-5 mb-3 block text-black dark:text-white">{messagesIntl.payreceivable.receivables.account_id} <span className="text-meta-1">*</span></label>
                                        <select onChange={(e) => setData({ ...data, account_id: e.target.value })} className="cursor-pointer relative z-20 w-full rounded border border-stroke bg-transparent py-3 px-5 outline-none transition focus:border-primary active:border-primary dark:border-form-strokedark dark:bg-form-input" required>
                                            ...
                                            {accounts.map((account, index) =>
                                                <option key={index} className="cursor-pointer disabled:bg-slate-200" value={account.id} disabled={(account.postable == 'N') ? 'disabled' : ''}>{account.name}</option>
                                            )}
                                        </select>
                                        <span className="mt-2 hidden text-sm text-red-500 peer-[&:not(:placeholder-shown):not(:focus):invalid]:block">
                                    ...
                                    <div className="form-control">
                                        <label className="pt-5 mb-3 block text-black dark:text-white">{messagesIntl.payreceivable.receivables.account_pay} <span className="text-meta-1">*</span></label>
                                        <select onChange={(e) => setData({ ...data, account_pay: e.target.value })} className="cursor-pointer relative z-20 w-full rounded border border-stroke bg-transparent py-3 px-5 outline-none transition focus:border-primary active:border-primary dark:border-form-strokedark dark:bg-form-input" required>
                                            <option value="" disabled selected='selected'>{messagesIntl.accounting.accounts.title}</option>
                                            {accountpays.map((account, index) =>
                                                <option key={index} className="cursor-pointer disabled:bg-slate-200" value={account.id} disabled={(account.postable == 'N') ? 'disabled' : ''}>{account.name}</option>
                                            )}
                                        </select>
                                        <span className="mt-2 hidden text-sm text-red-500 peer-[&:not(:placeholder-shown):not(:focus):invalid]:block">
